...
class SvgFont {
  ...
  //---------------------------------------------------------
  // Estimate string of text width using the parsed font
  estimateStringWidth(str, sca) {
    if (this.isReady()) {
      let cursorX = 0;
  ...
  //---------------------------------------------------------
  // Draw right justified string of text using the parsed font
  // x in this case is the right most point
  drawStringRight(str, x, y, sca) {
    if (this.isReady()) {
      let strWidth = this.estimateStringWidth(str, sca);
      let cursorX = x - strWidth;
      const scaleFactor = sca / this.unitsPerEm;
